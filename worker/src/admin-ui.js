  ...
  <main>
    ...
    <section id="app-view" class="app-shell hidden">
      <header class="topbar">
        ...
        <div class="header-actions"><button id="refresh-btn" class="ghost">重新整理</button><button id="logout-btn" class="ghost danger-text">登出</button></div>
      </header>
...
const editor=byId('editor-dialog'),passwordDialog=byId('password-dialog'),backupDialog=byId('backup-dialog');

async function api(path,options={}){
  const response=await fetch(path,{...options,headers:{'Content-Type':'application/json',...(options.headers||{})}});
  let data={};try{data=await response.json()}catch{}
  if(response.status===401&&path!=='/admin/api/login'){showLogin();throw new Error('管理登入已失效，請重新登入');}
